...
from tensorflow.keras.layers import Dense, Reshape, UpSampling2D, Conv2DTranspose, GlobalAveragePooling1D, Softmax, ZeroPadding1D
...
def dtc(x, y, nclusters):
    ...
    # Fit model
    t0 = time()
    dtc.fit(X_train=x, epochs=100, eval_epochs=5, save_epochs=100, batch_size=128, tol=0.001, patience=5, finetune_heatmap_at_epoch=8, save_dir='results/tmp')
    ...
    x_pred = q.argmax(axis=1)

    return x_pred,dtc.model
